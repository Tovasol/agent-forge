...
- BRIEF BOOTSTRAP ADDED: loop-init.ts initializes an idea directly FROM config/brief.json (niche/ICP/budget/stack/goal seeded into semantic memory + metrics) so the operator never retypes the idea. `forge idea` (bare) or `forge idea init` bootstraps; `--import` chains prior-work import; `--run` starts immediately; dedups by hint, `--force` overrides. Brief ICP seeded as provisional (does NOT auto-pass the niche gate; still validated live). Unit-tested.

- TWO-WAY DISCUSS ADDED: loop-discuss.ts — interactive REPL (`forge idea discuss <id> [--stage X]`) for back-and-forth on a stage. Agent holds brief + resume-derived operator profile + stage artifacts + facts/metrics/lessons; can defend/concede; logs exchange to episodic memory; ends with a proposed conclusion and ALWAYS asks before applying (y=apply+rerun, r=record only, n=keep talking) per operator policy. Wiring unit-tested.
